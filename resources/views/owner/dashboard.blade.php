...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 my-8">
    <div class="card shadow p-4 bg-white rounded-2xl">
        <h4 class="text-lg font-semibold mb-3">Grafik Bulanan per Driver</h4>
        <canvas id="chartBulananDriver"></canvas>
    </div>
    <div class="card shadow p-4 bg-white rounded-2xl">
        <h4 class="text-lg font-semibold mb-3">Grafik Tahunan per Driver</h4>
        <canvas id="chartTahunanDriver"></canvas>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const grafikBulananDriver = @json($grafikBulananDriver);
    const grafikTahunanDriver = @json($grafikTahunanDriver);
    ...
    // BULANAN DRIVER
    // ==============================
    const ctxBulanan = document.getElementById("chartBulananDriver");
    if (ctxBulanan && grafikBulananDriver) {
        const datasetsBulanan = [];
        Object.keys(grafikBulananDriver).forEach((id) => {
            const driverData = grafikBulananDriver[id];
            const namaDriver = driverData[0]?.nama_driver || "Driver " + id;
            const bulanLabels = driverData.map(d => d.bulan);
            const pendapatan = driverData.map(d => d.total_pendapatan);
    ...
    // TAHUNAN DRIVER
    // ==============================
    const ctxTahunan = document.getElementById("chartTahunanDriver");
    if (ctxTahunan && grafikTahunanDriver) {
        const datasetsTahunan = [];
        Object.keys(grafikTahunanDriver).forEach((id) => {
            const driverData = grafikTahunanDriver[id];
            const namaDriver = driverData[0]?.nama_driver || "Driver " + id;
            const tahunLabels = driverData.map(d => d.tahun);
            const pendapatan = driverData.map(d => d.total_pendapatan);
        ...
        new Chart(ctxTahunan, {
            type: "line",
            data: {
                labels: [...new Set([].concat(...Object.values(grafikTahunanDriver).map(v => v.map(d => d.tahun))))],
